...
- **adr-discipline** — Only write an Architecture Decision Record when **all three** are true: the decision is hard to reverse, it's surprising without context, an.
- **ubiquitous-language** — Maintain a `CONTEXT.md` in every project that defines the domain's shared language — every term that means something specific in this codeba.
